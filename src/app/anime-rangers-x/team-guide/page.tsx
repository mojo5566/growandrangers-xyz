...
export default function TeamGuidePage() {
  return (
    <ContentLayout title={data.title} description={data.description} breadcrumbs={data.breadcrumbs} accent="rangers">

      <section className="rounded-xl border border-[#252936] bg-[#14161D] p-4" aria-label="Last updated">
        <div className="flex items-center gap-2">
          <span className="text-sm">🕒</span>
          <p className="text-sm text-[#BAC4D1]"><strong className="text-white">Last Updated:</strong> {data.updatedAt}</p>
        </div>
      </section>

      <section aria-labelledby="overview-heading">
        <h2 id="overview-heading" className="font-heading text-[20px] font-semibold text-white lg:text-[24px] mb-4">👥 Why Team Composition Matters</h2>
        <div className="rounded-xl border border-[#252936] bg-[#14161D] p-5">
          ...
          <ul className="mt-4 space-y-2">
            {data.overview.highlights.map((h, i) => (
              <li key={i} className="flex items-start gap-2 text-xs text-[#BAC4D1]"><span className="text-[#FF3D00] shrink-0 mt-0.5">✔</span> {h}</li>
            ))}
          </ul>
        </div>
      </section>
      ...
      <section aria-labelledby="stages-heading">
        <h2 id="stages-heading" className="font-heading text-[20px] font-semibold text-white lg:text-[24px] mb-4">📈 Team Progression</h2>
      ...
      <section aria-labelledby="resources-heading">
        <h2 id="resources-heading" className="font-heading text-[20px] font-semibold text-white lg:text-[24px] mb-4">⚔️ Best Team Compositions</h2>
        <div className="grid gap-4 sm:grid-cols-2">
          {data.materials.map((m, i) => (
            <div key={i} className="rounded-xl border border-[#252936] bg-[#14161D] p-4">
              ...
              <div className="mb-3">
                <span className="text-xs font-semibold text-[#768294]">Traits:</span>
                <ul className="mt-1 space-y-0.5">
                  {m.sources.map((src, j) => (
                    <li key={j} className="text-xs text-[#768294] flex gap-1.5"><span className="text-[#FF3D00] shrink-0">+</span> {src}</li>
                  ))}
                </ul>
              </div>
      ...
      <section aria-labelledby="steps-heading">
        <h2 id="steps-heading" className="font-heading text-[20px] font-semibold text-white lg:text-[24px] mb-4">📋 Team Building Strategy</h2>
        <div className="rounded-xl border border-[#252936] bg-[#14161D] p-5">
          <ol className="space-y-3">
            {data.bestUnitsSteps.map((s, i) => (
              <li key={i} className="flex gap-3">
                <span className="text-[#FF3D00] font-bold text-lg shrink-0">{s.step}.</span>
                <div><h4 className="text-sm font-semibold text-white">{s.title}</h4><p className="mt-1 text-xs text-[#BAC4D1] leading-relaxed">{s.desc}</p></div>
              </li>
            ))}
          </ol>
        </div>
      ...
      <section aria-labelledby="priority-heading">
        <h2 id="priority-heading" className="font-heading text-[20px] font-semibold text-white lg:text-[24px] mb-4">⭐ Team Composition Tier List</h2>
        <div className="overflow-hidden rounded-xl border border-[#252936]">
          <div className="grid grid-cols-[60px_1fr_2fr] gap-2 bg-[#1E212B] px-4 py-2.5 items-center">
            ...
            <span className="text-xs font-semibold text-[#768294]">TEAM COMPOSITION</span>
      ...
      <section aria-labelledby="mistakes-heading">
        <h2 id="mistakes-heading" className="font-heading text-[20px] font-semibold text-white lg:text-[24px] mb-4">⚠️ Common Team Building Mistakes</h2>
        <div className="space-y-3">
          {data.mistakes.map((m, i) => (
            <div key={i} className="rounded-xl border border-[#252936] bg-[#14161D] p-4">
              <div className="flex items-start gap-3"><span className="text-[#FF3D00] text-lg shrink-0 font-bold">!</span><div><h3 className="text-sm font-semibold text-white">{m.title}</h3><p className="mt-1 text-xs text-[#BAC4D1] leading-relaxed">{m.desc}</p></div></div>
            </div>
          ))}
        </div>
      </section>
